...
export default class Matcher {
	...
	findMatches(item: Zotero.Item): number[] {
		...
		// DOI
		const doi = (
			Zotero.Utilities.cleanDOI(String(item.getField("DOI"))) ?? ""
		).toUpperCase();
		...
		for (const candidate of this._titleMap[title] ?? []) {
			...
			if (year && candidateYear && Math.abs(year - candidateYear) > 1)
				continue;
	...
	async _getDOIs() {
		...
		for (const row of rows) {
			...
			const newVal = (
				Zotero.Utilities.cleanDOI(String(row.value)) ?? ""
			).toUpperCase();
			if (!newVal) continue;
	...
	async _getYears() {
		...
		let sql =
			"SELECT itemID, SUBSTR(value, 1, 4) AS year FROM items " +
	...
	async _getCreators() {
		...
		let sql =
			"SELECT itemID, lastName, firstName, fieldMode FROM items " +
...
function parseCreator(
	...
	const firstInitial =
		creator.fieldMode === 0
			? normalizeString(creator.firstName).charAt(0)
			: false;
